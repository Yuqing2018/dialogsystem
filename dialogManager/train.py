...
def train_run():
    ...
    performance_records = {'success_rate': {},'avg_turns': {},'avg_reward': {}}
    while episode < NUM_EP_TRAIN:
        ...
        period_turn_total += turns
        # Train
        if episode % TRAIN_FREQ == 0:
            # Check success rate
            success_rate = period_success_total / TRAIN_FREQ
            avg_reward = period_reward_total / TRAIN_FREQ
            avg_turn = period_turn_total / TRAIN_FREQ
            ...
            if success_rate > success_rate_best:
                # 保存训练过程参数
                performance_records['success_rate'][episode] = success_rate
                performance_records['avg_turns'][episode] = avg_turn
                performance_records['avg_reward'][episode] = avg_reward
                print("episode：{}，NEW BEST success_rate：{}，avg_turns：{}，avg_reward：{}"
                      .format(episode, success_rate, avg_turn, avg_reward))
                save_performance_records("weights/", performance_records)
...
def save_performance_records(path, records):
    filename = 'dqn_agent_performance_records.json'
    filepath = os.path.join(path, filename)
    if not os.path.exists(filepath):
        os.system(r"touch {}".format(filepath))  # 调用系统命令行来创建文件
    try:
        # Serializing json
